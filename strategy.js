...
function scRowHtml(item) {
    ...
    <div class="border rounded-lg p-3 bg-gray-50" data-id="${item.id}">
      ...
      <div class="grid grid-cols-1 md:grid-cols-6 gap-3 items-start mt-3">
        <div class="md:col-span-1"><label class="block text-xs text-gray-600 mb-1">开始日期</label><input type="date" data-field="startDate" value="${item.startDate}" class="w-full p-2 border rounded-lg text-sm"></div>
        <div class="md:col-span-1 ${edGrey ? 'opacity-50 cursor-not-allowed' : ''}"><label class="block text-xs text-gray-600 mb-1">结束日期</label><input type="date" data-field="endDate" value="${edGrey ? item.startDate : item.endDate}" ${edGrey ? 'disabled' : ''} class="w-full p-2 border rounded-lg text-sm disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed"></div>
...
function simulateStrategy(item) {
    ...
    return { item, dates, assets, invests, cashDivs, holdAssets, holdCost, totalInvested, finalAsset, totalReturn, xirrVal, stopGainCount, maxPrincipal, maxPrincipalReturn, totalRedeemed, totalDividendCash, peakPrincipal, stopGainEvents, metrics: m };
}
...
function renderScResults(errors) {
    ...
    metricsGrid.innerHTML = scResults.map(r => {
        ...
        return `<div class="${cardBg} p-3 rounded-lg text-center">
            ...
            ${r.item.strategy === '5' ? `<div class="text-xs text-gray-400 mt-1">峰值本金收益率 ${r.maxPrincipalReturn.toFixed(2)}%（最大本金 ${r.maxPrincipal.toFixed(0)}）</div>` : ''}
        </div>`;
    }).join('');
    ...
    scChart = new Chart(canvas.getContext('2d'), {
        ...
        options: {
            ...
            scales: {
                ...
                y: { title: { display: true, text: hasStopGain
            ...
            plugins: {
                ...
                tooltip: { callbacks: { title: items => scChartXMode === 'date'
